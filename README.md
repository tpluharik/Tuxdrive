...
Version 0.3.0 targets Ubuntu 26.04. The installer resolves desktop dependencies automatically and TuxDrive securely downloads and verifies its pinned transfer engine on first launch if the system does not already provide a compatible one.
...
```bash
sudo apt install ./tuxdrive_0.3.0_all.deb
```
...
The installer is written to `dist/tuxdrive_0.3.0_all.deb`.
