...
Version 0.26.9 makes local collaboration approval-based: select a folder, advertise it on the LAN, and approve each nearby device when it requests access. File endpoints remain closed until approval, approved advertisements are scoped to the requesting identity, requests expire and are rate limited, and router port mapping is opt-in. Existing invitations, roles, Tor/relay paths, configuration, encrypted profiles, and synchronized content remain compatible.
...
The following controls are enforced in 0.26.9:
...
| --- | --- | --- |
| Ubuntu/Debian | `tuxindrive_0.26.9_all.deb` | Signed in-app Debian updates remain supported. |
| Windows 10/11 x64 | `TuxInDrive-0.26.9-windows-x64-setup.exe` | Same GTK desktop UI; install WinFsp for streaming drives. |
| macOS 12+ | `TuxInDrive-0.26.9-macos-*.dmg` | Same GTK desktop UI; install macFUSE for streaming drives. |
| Android 8+ | `TuxInDrive-0.26.9-android.apk` | Native phone/tablet UI, SAF folder access and OS-managed background sync. |
...
The Debian installer is written to `dist/tuxindrive_0.26.9_all.deb`. Windows, macOS and Android artifacts are built by `.github/workflows/platform-packages.yml` on their native build hosts. Durable packages are attached to the matching GitHub Release; dedicated signed channel manifests and package-location pointers live under [`releases/`](releases/README.md).
