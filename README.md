...
Version 0.20.4 targets Ubuntu 24.04/26.04 and Debian 12/13 GNOME on amd64 and arm64. Streaming mounts remain files-on-demand after reconnect: saved pins are verified from local cache markers without downloading cloud content, while explicit **Keep available offline** and **Free local space (make online-only)** actions control each item. Individual files now wait for rclone's exact complete cache object before receiving an offline marker, and Nautilus routes their paths without resolving or statting FUSE content. A credential-free Nautilus snapshot keeps the complete TuxDrive menu available inside mounted folders, and the app can reset a whole drive to online-only mode. It retains editable internal folder groups, GitHub synchronization, the 0.19 security baseline, unbranded six-state Nautilus badges, searchable offline documentation and persistent English, German, French, Spanish, Arabic and Hebrew localization.
...
The following controls are enforced in 0.20.4:
...
### 0.20.4 online-only/offline availability, groups and GitHub
...
The installer is written to `dist/tuxdrive_0.20.4_all.deb`. TuxDrive publishes Debian packages only.
...
The current suite contains 148 automated tests, including lexical single-file action routing, delayed exact VFS cache publication, nested online-only exceptions, local no-download reconnect verification, pin-aware VFS retention/remounting, verified badge state, root/item hydration and rollback, GitHub URL/branch/command guards, synchronized-folder group migration, unbranded Nautilus SVG validation, legacy/v2 trust-channel validation, updater race-boundary, per-device peer isolation, hostile ODF/CRDT input and six-language help parity. See [Testing and release verification](docs/TESTING.md) for details.
