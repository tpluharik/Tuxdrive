...
Version 0.26.8 introduces the circular penguin visual identity across every supported platform and repository surface. It retains the credential-enabled profile backup, restore, and QR export repair delivered in 0.26.7, secure desktop-to-Android QR transfer from 0.26.6, and the application-wide bandwidth controller from 0.26.5. Existing installations continue to read their private legacy configuration/state directories, encrypted profile format, peer invitations, and hidden synchronization metadata in place; no cloud or local content is renamed.
...
The following controls are enforced in 0.26.8:
...
| --- | --- | --- |
| Ubuntu/Debian | `tuxindrive_0.26.8_all.deb` | Signed in-app Debian updates remain supported. |
| Windows 10/11 x64 | `TuxInDrive-0.26.8-windows-x64-setup.exe` | Same GTK desktop UI; install WinFsp for streaming drives. |
| macOS 12+ | `TuxInDrive-0.26.8-macos-*.dmg` | Same GTK desktop UI; install macFUSE for streaming drives. |
| Android 8+ | `TuxInDrive-0.26.8-android.apk` | Native phone/tablet UI, SAF folder access and OS-managed background sync. |
...
The Debian installer is written to `dist/tuxindrive_0.26.8_all.deb`. Windows, macOS and Android artifacts are built by `.github/workflows/platform-packages.yml` on their native build hosts. Durable packages are attached to the matching GitHub Release; dedicated signed channel manifests and package-location pointers live under [`releases/`](releases/README.md).
