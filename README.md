...
Version 0.20.5 targets Ubuntu 24.04/26.04 and Debian 12/13 GNOME on amd64 and arm64. Streaming mounts remain files-on-demand after reconnect: saved pins are verified from local cache markers without downloading cloud content, while explicit **Keep available offline** and **Free local space (make online-only)** actions control each item. A single-file action now hydrates and tags only that exact file; it never remounts the FUSE drive or reads neighbouring files. The Nautilus extension retains its last complete credential-free job snapshot so the full menu remains available through atomic state refreshes. TuxDrive also retains editable internal folder groups, GitHub synchronization, the 0.19 security baseline, unbranded six-state Nautilus badges, searchable offline documentation and persistent English, German, French, Spanish, Arabic and Hebrew localization.
...
The following controls are enforced in 0.20.5:
...
### 0.20.5 online-only/offline availability, groups and GitHub
...
The installer is written to `dist/tuxdrive_0.20.5_all.deb`. TuxDrive publishes Debian packages only.
...
The current suite contains 151 automated tests, including lexical single-file action routing, sibling-file rule isolation, delayed exact VFS cache publication, stable non-remounting retention, last-known-good Nautilus menu metadata, nested online-only exceptions, local no-download reconnect verification, verified badge state, root/item hydration and rollback, GitHub guards, synchronized-folder groups, update trust, peer isolation, hostile ODF/CRDT input and six-language help parity. See [Testing and release verification](docs/TESTING.md) for details.
