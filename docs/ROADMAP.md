...
## Current baseline: 0.20.4

Version 0.20.4 restores explicit files-on-demand behavior after reconnect and fixes individual-file pinning by waiting for rclone's exact complete cache object before publishing success. Saved pins are verified from local cache manifests without opening cloud files; only a user action downloads content. Online-only child exceptions can override pinned parents, the app exposes a whole-drive recovery control, and Nautilus uses lexical FUSE-safe path routing plus an atomic credential-free job snapshot. It retains pin-aware cache policy, editable metadata-only groups and GitHub repository synchronization through credential-free system Git integration. GitHub two-way jobs commit, fetch, safely rebase and push; guarded one-way modes and explicit conflict/authentication errors prevent silent history loss. The release also retains the six functional Nautilus badges, complete 0.19.0 critical/high remediation, signed 0.19.1 trust bridge, six-language documentation and collaboration baseline.
...
| 11 | Per-file offline availability controls | Cloud | Fixed 0.20.4 | Streaming roots/files/folders expose **Keep available offline** and **Free local space (make online-only)**. Individual files use lexical FUSE-safe routing and wait for the exact complete rclone cache object; nested online-only exceptions override pinned parents, the first/last pin reapplies the live VFS policy, and reconnect verifies local manifests without downloading cloud content. Green per-item state requires local cache verification. |
| 12 | Nautilus integration | Cloud | Repaired 0.20.4 | Live state emblems, safe sync/web/log actions and per-item streaming availability controls are shipped. An atomic credential-free snapshot prevents full-menu loss inside FUSE folders. Six unbranded, color/shape/glyph-distinct functional badges make pending, synchronizing, synchronized, files-on-demand, paused, and error states immediately visible. Integration is optional in Settings and enabled by default. |
