...
## Current baseline: 0.20.2

Version 0.20.2 makes streaming offline retention durable rather than relying on a one-time VFS read. The first pin applies a live pin-aware mount policy, persisted rules are rehydrated after every reconnect, redundant child rules are normalized, and green badges are published only after the current mount verifies complete hydration. It retains reliable Nautilus dispatch, editable metadata-only groups and GitHub repository synchronization through credential-free system Git integration. GitHub two-way jobs commit, fetch, safely rebase and push; guarded one-way modes and explicit conflict/authentication errors prevent silent history loss. The release also retains the six functional Nautilus badges, complete 0.19.0 critical/high remediation, signed 0.19.1 trust bridge, six-language documentation and collaboration baseline.
...
| 11 | Per-file offline availability controls | Cloud | Fixed 0.20.2 | Streaming roots/files/folders expose **Always keep available offline** and **Free local space**. Registered in-process actions dispatch immediately for active mounts, cold-start requests queue until readiness, the first/last pin reapplies the live VFS retention policy, and persisted rules rehydrate after reconnect. Green per-item state requires current-mount verification; stale/failed rules remain unconfirmed with a visible error. |
| 12 | Nautilus integration | Cloud | UI refined 0.19.2 | Live state emblems, safe sync/web/log actions and per-item streaming availability controls are shipped. Six unbranded, color/shape/glyph-distinct functional badges make pending, synchronizing, synchronized, files-on-demand, paused, and error states immediately visible. Integration is optional in Settings and enabled by default. |
