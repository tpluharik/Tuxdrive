...
## Current baseline: 0.20.3

Version 0.20.3 restores explicit files-on-demand behavior after reconnect. Saved pins are verified from local cache manifests without opening cloud files; only a user action downloads content. Online-only child exceptions can override pinned parents, the app exposes a whole-drive recovery control, and Nautilus consumes an atomic credential-free job snapshot so its menu remains available inside FUSE folders. It retains pin-aware cache policy, editable metadata-only groups and GitHub repository synchronization through credential-free system Git integration. GitHub two-way jobs commit, fetch, safely rebase and push; guarded one-way modes and explicit conflict/authentication errors prevent silent history loss. The release also retains the six functional Nautilus badges, complete 0.19.0 critical/high remediation, signed 0.19.1 trust bridge, six-language documentation and collaboration baseline.
