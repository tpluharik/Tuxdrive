...
## Current baseline: 0.20.1

Version 0.20.1 makes streaming offline retention reliably dispatched from Nautilus for both roots and nested items, including mounted-drive fast paths, cold-start queuing and visible failures. It retains editable metadata-only groups for synchronized folders and GitHub repository synchronization through credential-free system Git integration. GitHub two-way jobs commit, fetch, safely rebase and push; guarded one-way modes and explicit conflict/authentication errors prevent silent history loss. The release also retains the six functional Nautilus badges, complete 0.19.0 critical/high remediation, signed 0.19.1 trust bridge, six-language documentation and collaboration baseline.
...
| 11 | Per-file offline availability controls | Cloud | Fixed 0.20.1 | Streaming roots/files/folders expose **Always keep available offline** and **Free local space**. Registered in-process actions dispatch immediately for active mounts, cold-start requests queue until mount readiness, and persistent rules hydrate VFS content. Per-item blue/green Nautilus state distinguishes active hydration from confirmed offline availability, and failed hydration rolls back the pin with a visible notification. |
