...
Version 0.15.0 is the current documented and packaged release. It adds Tor v3 workspace transport, per-device Onion authorization, fail-closed privacy policies, and private bridge/pluggable-transport profiles. Version 0.14.0 added encrypted configuration backup and device migration. Ranks 1–19 and 21–24 are implemented; rank 20 and ranks 25–40 remain portability, secure-workspace, optional-server and local-first collaboration work.
...
| 21 | Tor v3 Onion Service transport | Privacy | Completed 0.15.0 | Isolated persistent/ephemeral services expose only the pinned SFTP endpoint; Tor-only startup and connection fail closed without clearnet fallback. |
...
| 23 | Fail-closed transport and anonymity policies | Privacy | Completed 0.15.0 | Per-workspace direct/Tor-only, no-relay, no-public-IP and never-provider-cloud fields block incompatible transport and create actionable private audit events. |
| 24 | Tor bridges and pluggable-transport profiles | Privacy | Completed 0.15.0 | Advanced profiles live only in mode-0600 isolated Tor configuration and are excluded from invitations, process arguments and application logs; UI warns that censorship resistance is not anonymity proof. |
