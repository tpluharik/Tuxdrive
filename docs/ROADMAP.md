...
Version 0.20.0 makes streaming offline retention operational and visible for both roots and nested items, adds editable metadata-only groups for synchronized folders, and adds GitHub repository synchronization through credential-free system Git integration. GitHub two-way jobs commit, fetch, safely rebase and push; guarded one-way modes and explicit conflict/authentication errors prevent silent history loss. The release retains the six functional Nautilus badges, complete 0.19.0 critical/high remediation, signed 0.19.1 trust bridge, six-language documentation and collaboration baseline.
...
| 11 | Per-file offline availability controls | Cloud | Fixed 0.20.0 | Streaming roots/files/folders expose **Always keep available offline** and **Free local space**. Persistent rules hydrate VFS content and prevent normal age eviction; per-item blue/green Nautilus state distinguishes active hydration from confirmed offline availability, and failed hydration rolls back the pin. |
