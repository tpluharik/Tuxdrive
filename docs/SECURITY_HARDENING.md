...
| Area | 0.26.10 behavior | Security purpose |
|---|---|---|
...
| CI/release | Pinned release dependencies, 326 Python/Android tests, compile checks, high-severity Bandit, `pip-audit`, Debian/platform inspection, CycloneDX SBOM, signed-manifest verification | Make security regressions and vulnerable dependencies release blockers |
...
2. Confirm the running version is 0.26.10 and the platform update check reports a valid signature, origin, filename, digest, size, architecture and expiry.
