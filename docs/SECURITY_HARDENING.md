...
| Area | 0.20.4 behavior | Security purpose |
|---|---|---|
...
| Offline hydration | Root/child confinement, symlink rejection, failed-pin rollback, live pin-aware eviction policy, explicit nested online-only exceptions, and confined local pin manifests checked without remote reads | Avoid silent reconnect downloads, false offline claims, generic-cache eviction of pinned content, and path escape while caching streamed content |
| CI/release | Pinned GitHub Action SHAs, 148 tests, compile checks, high-severity Bandit, `pip-audit`, Debian inspection, CycloneDX SBOM, signed-manifest verification | Make security regressions and vulnerable dependencies release blockers |
...
2. Confirm the running version is 0.20.4 and the update check reports a valid signature and expiry.
