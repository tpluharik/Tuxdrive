...
| Area | 0.26.13 behavior | Security purpose |
|---|---|---|
...
| CI/release | Pinned release dependencies, 346 Python tests plus 11 Android JVM tests, compile checks, high-severity Bandit, `pip-audit`, Debian client/server installation matrices, CycloneDX SBOM, signed-manifest verification | Make security regressions, broken installers and vulnerable dependencies release blockers |
...
2. Confirm the running version is 0.26.13 and the platform update check reports a valid signature, origin, filename, digest, size, architecture and expiry.
