# TuxDrive 0.20.2 security hardening and secure operation

This document explains the controls retained through TuxDrive 0.20.2, including the second-round critical/high remediation, durable verified offline retention, GitHub synchronization boundaries, what changed for existing users, which data remain sensitive, what the controls do not guarantee, and how maintainers verify a release. It complements the concise vulnerability-reporting policy in [`SECURITY.md`](../SECURITY.md).

## Supported baseline and immediate action

Version **0.20.2** is the supported baseline. It retains root-side update re-verification, per-key peer endpoints, isolated send/drop roots and bounded ODF/CRDT parsing while adding GitHub URL/branch validation, system-owned Git credentials, hydration rollback, reliable Nautilus action dispatch, pin-aware live VFS retention and current-mount verification before green offline status. Python/PyPI installations require `cryptography>=50.0.0,<51`; Debian installations use the distribution-maintained `python3-cryptography` package so vendor backports remain valid.
...
| Area | 0.20.2 behavior | Security purpose |
|---|---|---|
...
| Offline hydration | Root/child confinement, symlink rejection, failed-pin rollback, live pin-aware eviction policy, reconnect rehydration, and current-mount verified item state | Avoid false offline claims, generic-cache eviction of pinned content, and path escape while caching streamed content |
| CI/release | Pinned GitHub Action SHAs, 140 tests, compile checks, high-severity Bandit, `pip-audit`, Debian inspection, CycloneDX SBOM, signed-manifest verification | Make security regressions and vulnerable dependencies release blockers |
...
2. Confirm the running version is 0.20.2 and the update check reports a valid signature and expiry.
