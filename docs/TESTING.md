...
| `test_config.py` | 5 | Round-trip persistence, bounded cache-setting validation, unchanged-write suppression, private `0600` permissions, and invalid configuration quarantine. |
...
dpkg-deb --info dist/tuxdrive_0.24.1_all.deb
dpkg-deb --contents dist/tuxdrive_0.24.1_all.deb
sha256sum dist/tuxdrive_0.24.1_all.deb
```
...
```bash
python3 scripts/sign-update.py --version 0.24.1 \
  --package dist/tuxdrive_0.24.1_all.deb \
