...
The TuxDrive 0.20.8 suite contains **160 automated tests**. Tests use temporary directories and mocked cloud/Git/Tor processes where possible, so they do not require or expose real OAuth or GitHub tokens, cloud accounts, Onion credentials, peer identities, vault passwords, presence passphrases, or personal files. The updater tests validate the fixed original-key 0.19.1 bridge and the rotated-key v2 channel against the exact current package.
...
| `test_nautilus_extension.py` | 6 | Exact sibling/parent exclusion, background-menu isolation, last-known-good job/badge metadata, coalesced metadata invalidation, stale-handle removal, and preservation of the TuxDrive menu when a file changes from pending to verified offline. |
...
dpkg-deb --info dist/tuxdrive_0.20.8_all.deb
dpkg-deb --contents dist/tuxdrive_0.20.8_all.deb
sha256sum dist/tuxdrive_0.20.8_all.deb
```
...
```bash
python3 scripts/sign-update.py --version 0.20.8 \
  --package dist/tuxdrive_0.20.8_all.deb \
