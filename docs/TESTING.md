...
The TuxDrive 0.7.0 suite contains **53 automated tests**. Tests use temporary directories and mocked cloud processes where possible, so they do not require or expose real OAuth tokens, cloud accounts, peer identities, or personal files.
...
dpkg-deb --info dist/tuxdrive_0.7.0_all.deb
dpkg-deb --contents dist/tuxdrive_0.7.0_all.deb
sha256sum dist/tuxdrive_0.7.0_all.deb
```

The build script performs an additional import smoke test against the exact staged `/usr/lib` layout used after installation. It verifies the TuxDrive version and confirms that the desktop application, updater and peer modules are discoverable.
