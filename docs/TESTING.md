...
The TuxDrive 0.20.4 suite contains **148 automated tests**. Tests use temporary directories and mocked cloud/Git/Tor processes where possible, so they do not require or expose real OAuth or GitHub tokens, cloud accounts, Onion credentials, peer identities, vault passwords, presence passphrases, or personal files. The updater tests validate the fixed original-key 0.19.1 bridge and the rotated-key v2 channel against the exact current package.
...
| `test_engine.py` | 30 | Two-way initialization, one-way direction, rename tracking, deletion ceilings, conflict flags, incremental transfers, streaming commands, safe overlap, mount recovery, pin-aware eviction policy/remounting, offline/online-only rule normalization, root/item hydration, delayed exact single-file cache publication, local marker verification without remote reads, cache release, marker confinement, failed-pin rollback, symlink rejection, failure summaries and transfer-engine replacement. |
...
| `test_offline_action.py` | 8 | Mounted-drive fast dispatch, cold-start queuing, both supported command-line availability option forms, lexical file routing without FUSE resolution, sibling-prefix rejection, nested offline/online-only precedence, and green-state publication only for locally verified rules. |
...
dpkg-deb --info dist/tuxdrive_0.20.4_all.deb
dpkg-deb --contents dist/tuxdrive_0.20.4_all.deb
sha256sum dist/tuxdrive_0.20.4_all.deb
```
...
```bash
python3 scripts/sign-update.py --version 0.20.4 \
  --package dist/tuxdrive_0.20.4_all.deb \
