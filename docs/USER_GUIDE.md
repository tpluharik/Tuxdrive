...
This guide covers TuxInDrive 0.26.20 on Linux, Windows, macOS and Android. Windows and macOS retain the Linux GTK desktop layout, while Android reorganizes accounts, synchronized folders, cloud files, activity and settings for touch displays. Platform-specific installation and signing details are in [Platform support](PLATFORM_SUPPORT.md). Administrators and developers can continue with the [documentation index](README.md), [operations guide](OPERATIONS.md), and [architecture reference](ARCHITECTURE.md).
...
Version 0.26.20 is the supported security baseline. Upgrade older installations before reconnecting cloud or peer accounts. See [Security hardening and secure operation](SECURITY_HARDENING.md) for the complete control inventory and post-upgrade checklist.
...
Reinstall the current package with `sudo apt install ./tuxindrive_0.26.20_all.deb`.
...
### Security upgrade checklist for 0.26.20

1. Install `tuxindrive_0.26.20_all.deb`; the upgrade closes an older running TuxInDrive instance. Reopen TuxInDrive and restart Nautilus.
2. Confirm **Settings → Check for updates** reports 0.26.20 and no signature or expiry error.
