...
Version 0.21.1 is the supported security baseline. Upgrade older installations before reconnecting cloud or peer accounts. See [Security hardening and secure operation](SECURITY_HARDENING.md) for the complete control inventory and post-upgrade checklist.
...
1. Press the three-line handle, move the pointer until the folder drag icon follows it, and release above or below another visible folder to change its saved position. Version 0.21.1 uses GTK's recognized same-application UTF-8 target so the drop receives the folder identifier.
...
Reinstall the current package with `sudo apt install ./tuxdrive_0.21.1_all.deb`.
...
### Security upgrade checklist for 0.21.1

1. Install `tuxdrive_0.21.1_all.deb`; the upgrade closes an older running TuxDrive instance. Reopen TuxDrive and restart Nautilus.
2. Confirm **Settings → Check for updates** reports 0.21.1 and no signature or expiry error.
