...
Version 0.26.7 is the supported security baseline. Upgrade older installations before reconnecting cloud or peer accounts. See [Security hardening and secure operation](SECURITY_HARDENING.md) for the complete control inventory and post-upgrade checklist.
...
```bash
sudo apt install ./tuxindrive_0.26.7_all.deb
```
...
Reinstall the current package with `sudo apt install ./tuxindrive_0.26.7_all.deb`.
...
### Security upgrade checklist for 0.26.7

1. Install `tuxindrive_0.26.7_all.deb`; the upgrade closes an older running TuxInDrive instance. Reopen TuxInDrive and restart Nautilus.
2. Confirm **Settings → Check for updates** reports 0.26.7 and no signature or expiry error.
