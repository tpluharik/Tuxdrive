...
Version 0.20.0 is the supported security baseline. Upgrade older installations before reconnecting cloud or peer accounts. See [Security hardening and secure operation](SECURITY_HARDENING.md) for the complete control inventory and post-upgrade checklist.
...
```bash
sudo apt install ./tuxdrive_0.20.0_all.deb
```
...
Reinstall the current package with `sudo apt install ./tuxdrive_0.20.0_all.deb`.
...
### Security upgrade checklist for 0.20.0

1. Install `tuxdrive_0.20.0_all.deb` and restart TuxDrive and Nautilus.
2. Confirm **Settings → Check for updates** reports 0.20.0 and no signature or expiry error.
