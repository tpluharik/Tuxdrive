"""Privileged, fail-closed installation of a signed TuxDrive update.
...
from .updater import MANIFEST_URL, MAX_UPDATE_SIZE, UpdateManager
...
def _signed_release():
    request = urllib.request.Request(MANIFEST_URL, headers={"User-Agent": "TuxDrive-Privileged-Updater"})
...
def stage_verified_package(source: Path, destination: Path, release) -> Path:
    expected_name = f"tuxdrive_{release.version}_all.deb"
    if source.name != expected_name:
        raise PrivilegedUpdateError("The selected package is not the signed current TuxDrive release")
...
def install(package: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="tuxdrive-update-", dir="/var/tmp") as directory:
        root = Path(directory)
        os.chmod(root, 0o700)
        staged = stage_verified_package(package, root / f"tuxdrive_{release.version}_all.deb", release)
        ...
        if metadata.returncode or metadata.stdout.strip() != f"tuxdrive {release.version}":
            raise PrivilegedUpdateError("The signed file is not the expected TuxDrive Debian package")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(values) != 1:
        print("Usage: tuxdrive-update-helper PACKAGE.deb", file=sys.stderr)
        return 2
    try:
        install(Path(values[0]))
    except (OSError, ValueError, PrivilegedUpdateError) as exc:
        print(f"TuxDrive update refused: {exc}", file=sys.stderr)
        return 1
    return 0
