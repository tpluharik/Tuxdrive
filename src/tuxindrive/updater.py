...
MANIFEST_URL = "https://raw.githubusercontent.com/tpluharik/TuxInDrive/main/update/latest-v2.json"
ALLOWED_PREFIXES = (
...
def release_package_name(release: UpdateRelease) -> str:
...
class UpdateManager:
    ...
    def parse_manifest(payload: bytes, public_key: str = UPDATE_PUBLIC_KEY) -> UpdateRelease:
        ...
        if not any(release.url.startswith(prefix) for prefix in ALLOWED_PREFIXES) or not release.url.endswith(".deb"):
    ...
    def check(self) -> UpdateRelease | None:
        request = urllib.request.Request(MANIFEST_URL, headers={"User-Agent": "TuxInDrive-Updater"})
    ...
    def download(
        ...
        target = self.cache_dir / release_package_name(release)
